    ...
    const providerProfiles = {
      ...
      anthropic: {
        ...
        creditsLabel: 'Codex Credits',
        creditsUnavailable: 'Codex credits apply only to Codex/OpenAI usage.',
    ...
    function loadedRowsCaveat(range) {
      ...
      return `${scope} totals are from loaded rows only (${loadedRowsDescription()}). Run codex-usage-tracker serve-dashboard to load every call in range.`;
    }
    ...
    function updateAllowanceSourceLine() {
      ...
      sourceEl.title = [
        ...
        allowanceWindows.length ? `Allowance windows: ${allowanceWindows.map(window => short(window.label || window.key)).join(', ')}. Remaining window source: ${windowSourceName}.` : 'Run codex-usage-tracker init-allowance to add remaining usage windows.',
    ...
    function updatePromptLine() {
      ...
      promptEl.innerHTML = `<span class="prompt-path">codex-usage-tracker:~$</span> ${escapeHtml(echo)}`;
    }
    function render() {
    ...
    function rowDetailStatus(row) {
      ...
      return 'Open this dashboard with codex-usage-tracker serve-dashboard to load additional aggregate fields.';
    }
