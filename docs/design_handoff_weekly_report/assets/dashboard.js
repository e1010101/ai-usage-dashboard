    ...
    const analyticsDatePresetEl = document.getElementById('analyticsDatePreset');
    const analyticsDateStartEl = document.getElementById('analyticsDateStart');
    const analyticsDateEndEl = document.getElementById('analyticsDateEnd');
    ...
    const trendBucketEl = document.getElementById('trendBucket');
    ...
    const allowedTrendBuckets = new Set(['auto', 'day', 'week', 'month']);
    ...
    const dailyTrendPresets = new Set(['this-week', 'last-7-days', 'this-month', 'last-30-days', 'last-90-days', 'this-year']);
    ...
    function trendUnitFor(startMs, endMs, preset = '', trendBucket = 'auto') {
      const spanDays = (endMs - startMs) / 86400000;
      if (trendBucket !== 'auto' && allowedTrendBuckets.has(trendBucket)) return trendBucket;
    ...
    function buildUsageTrend(rows, dateRange) {
      ...
      const trendBucket = allowedTrendBuckets.has(trendBucketEl.value) ? trendBucketEl.value : 'auto';
      const unit = trendUnitFor(startMs, endMs, preset, trendBucket);
    ...
    function applyInitialState() {
      ...
      if (allowedTrendBuckets.has(initialState.trendBucket)) trendBucketEl.value = initialState.trendBucket;
    ...
    function inclusiveEndDate(endExclusive) {
    ...
    function syncDatePresetMirrors() {
      analyticsDatePresetEl.value = datePresetEl.value;
    }
    function setDateInputValues(startValue, endValue) {
      dateStartEl.value = startValue;
      dateEndEl.value = endValue;
      analyticsDateStartEl.value = startValue;
      analyticsDateEndEl.value = endValue;
    }
    function syncDateInputMirrors() {
      analyticsDateStartEl.value = dateStartEl.value;
      analyticsDateEndEl.value = dateEndEl.value;
    }
    function presetDateRange(preset) {
      const now = new Date();
      const today = localDay(now);
      if (preset === 'past-day') {
        return { start: new Date(now.getTime() - 86400000), endExclusive: now };
      }
      if (preset === 'today') {
      ...
      if (preset === 'yesterday') {
        ...
        return { start, endExclusive: today };
      }
      if (preset === 'this-week') {
    ...
    function syncDatePresetInputs() {
      ...
      setDateInputValues(
        range.start ? localDateKey(range.start) : '',
        range.endExclusive ? localDateKey(inclusiveEndDate(range.endExclusive)) : '',
      );
    }
    function toggleCustomDateFields() {
      const custom = datePresetEl.value === 'custom';
      [dateStartEl, dateEndEl, analyticsDateStartEl, analyticsDateEndEl].forEach(el => {
        const field = el.closest('.date-field-custom');
        const analyticsField = el.closest('.analytics-date-custom');
        if (field) field.hidden = !custom;
        if (analyticsField) analyticsField.hidden = !custom;
        el.disabled = !custom;
      });
    }
    ...
    function presetDateRangeLabel(preset, range) {
      if (preset === 'past-day') return datePresetLabels[preset];
      return formatDateRangeLabel(datePresetLabels[preset], range.start, inclusiveEndDate(range.endExclusive));
    }
    function currentDateRange() {
      ...
      if (preset !== 'custom' && preset !== 'all') {
        ...
        return {
          ...
          label: presetDateRangeLabel(preset, range),
        };
      }
    ...
    function currentDashboardState() {
      return {
        ...
        trendBucket: trendBucketEl.value,
    ...
    trendBucketEl.addEventListener('change', () => {
      syncUrlState();
      render();
    });
    ...
    function applyDatePresetSelection(value) {
      datePresetEl.value = allowedDatePresets.has(value) ? value : 'all';
      syncDatePresetInputs();
      toggleCustomDateFields();
      currentPage = 1;
      render();
    ...
    function applyCustomDateInput(sourceEl, targetEl) {
      if (datePresetEl.value !== 'custom') datePresetEl.value = 'custom';
      targetEl.value = cleanDateInput(sourceEl.value) || sourceEl.value;
      syncDatePresetMirrors();
      syncDateInputMirrors();
      toggleCustomDateFields();
      currentPage = 1;
    ...
    [datePresetEl, analyticsDatePresetEl].forEach(el => el.addEventListener('input', () => applyDatePresetSelection(el.value)));
    [
      [dateStartEl, dateStartEl],
      [dateEndEl, dateEndEl],
      [analyticsDateStartEl, dateStartEl],
      [analyticsDateEndEl, dateEndEl],
    ].forEach(([sourceEl, targetEl]) => sourceEl.addEventListener('input', () => applyCustomDateInput(sourceEl, targetEl)));
