...
- `parser.py` is the compatibility facade for source adapters that convert local source data into aggregate `UsageEvent` records. It must not persist prompts, assistant text, tool output, or transcript snippets.
- `adapters/base.py` defines the source-adapter protocol. `adapters/codex_jsonl.py` owns Codex JSONL parsing from `~/.codex/sessions`, `adapters/claude_code_jsonl.py` owns Claude Code JSONL parsing from `~/.claude/projects`, and `adapters/hermes_state_db.py` owns Hermes aggregate SQLite parsing from `state.db`. New sources should add an adapter instead of adding provider-specific logic to store, reports, dashboard, or MCP code.
...
- `costing.py`, `pricing_config.py`, `pricing_openai.py`, `pricing_deepseek.py`, `pricing_estimates.py`, and `allowance.py` own cost, credit, rate-card, and allowance annotation. Keep estimate confidence and source metadata attached to rows. Codex credit estimates are Codex/OpenAI-only; non-Codex rows should be marked not applicable rather than mapped through Codex rate cards.
